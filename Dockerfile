...
CMD ln -s /usr/bin/nodejs /usr/bin/node && git clone https://github.com/PabloRal/hopsiup.git && cd hopsiup && \
    git checkout deploy && virtualenv venv && . venv/bin/activate && cd hopsiup && \
    pip install -r requirements.txt && bower --allow-root install &&\
    ./init_db.sh && ./fill_db.sh && python hopsiup.py
